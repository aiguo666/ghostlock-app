...
def mm_slab_object_size(sz: int, release: str | None) -> int:
    """mm_struct kmem_cache object size: sizeof(mm_struct) plus the trailing
    per-cpu cpumask (and mm_cid on 6.x), rounded up to the cache line. This
    is what kernelsnitch uses as its mm_struct scan stride, not sizeof()
    alone. Phones (<= 64 CPUs) always yield 0x500 on 6.x and 0x400 on 5.15;
    larger CPU counts need BITS_TO_LONGS(nr_cpu_ids)*8 per mask."""
    is_5_15 = bool(release and release.startswith("5.15"))
    cpumask = 8  # BITS_TO_LONGS(<= 64 cpus) * sizeof(long)
    extra = cpumask if is_5_15 else 2 * cpumask  # + mm_cid on 6.x
    return (sz + extra + 63) & ~63
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if struct_offsets.get("struct_mm_struct") is not None:
            struct_offsets["struct_mm_struct"] = mm_slab_object_size(
                struct_offsets["struct_mm_struct"], boot.release()
            )
        ...
        if mm_size is not None:
            print(
                f"info: mm_struct slab object size=0x{mm_size:X} "
                "(sizeof + cpumask + mm_cid, cache-line aligned; "
                "kernelsnitch scan stride)",
                file=sys.stderr,
            )
            if mm_size > 0x500:
                print(
                    "warning: mm_struct slab object size exceeds the "
                    "STRUCT_OFFSETS_* macro value (0x500)",
                    file=sys.stderr,
                )
        report = {
